...
export default function BlogPage() {
  ...
  return (
    <main className="min-h-dvh font-sans bg-(--background) text-(--foreground) selection:bg-emerald-100">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center group cursor-pointer">
                    ...
                    <div>
                        <div className="flex gap-3 mb-6">
                            <span className="px-3 py-1 bg-(--card) border border-(--border) rounded-full text-xs font-bold text-(--muted) uppercase tracking-wide">
                                {featuredPost.category}
                            </span>
                            <span className="px-3 py-1 bg-transparent text-xs font-medium text-(--muted)">
                                {featuredPost.readTime}
                            </span>
                        </div>
                        
                        <h1 className="text-4xl md:text-6xl font-display font-bold text-(--foreground) mb-6 leading-[1.1] group-hover:text-emerald-700 transition-colors">
                            {featuredPost.title}
                        </h1>
                        ...
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-(--border) overflow-hidden relative">
                                <Image src={`https://ui-avatars.com/api/?name=${featuredPost.author}&background=0f172a&color=fff`} alt={featuredPost.author} fill />
                            </div>
                            <div className="text-sm">
                                <p className="font-bold text-(--foreground)">{featuredPost.author}</p>
                                <p className="text-(--muted)">{featuredPost.date}</p>
                            </div>
                        </div>
                    </div>
                </div>
      ...
      <section className="sticky top-20 z-30 bg-(--card)/80 backdrop-blur-xl border-b border-(--border) py-4 px-6">
         <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4 px-4 sm:px-6 lg:px-8">
            <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto no-scrollbar">
                {CATEGORIES.map(cat => (
                    <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all duration-(--duration-150) ${
                            activeCategory === cat
            ...
            <div className="relative w-full md:w-64">
                ...
                <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-(--surface) border border-(--border) rounded-full text-sm text-(--foreground) placeholder:text-(--muted) focus:outline-none focus:border-(--emerald) transition-colors duration-(--duration-150)"
                />
            </div>
         </div>
      </section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {filteredPosts.filter(p => !p.featured).map((post) => (
                    <Link href={`/blog/${post.slug}`} key={post.id} className="group flex flex-col h-full">
                        <div className="relative aspect-[16/10] rounded-2xl overflow-hidden mb-6 bg-(--surface)">
                            <Image 
                                src={post.image} 
                                alt={post.title} 
                                fill 
                            ...
                            />
                            <div className="absolute top-4 left-4 bg-(--card)/90 backdrop-blur-md px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider text-(--foreground)">
                                {post.category}
                            </div>
                        </div>
                        
                        <div className="flex-1 flex flex-col">
                            <h3 className="text-2xl font-bold text-(--foreground) mb-3 leading-tight group-hover:text-emerald-700 transition-colors">
                                {post.title}
                            </h3>
                            <p className="text-(--muted) leading-relaxed mb-6 line-clamp-3 flex-1">
                                {post.excerpt}
                            </p>
                            ...
                            <div className="flex items-center justify-between border-t border-(--border) pt-4 mt-auto">
                                ...
                                <ArrowRight size={16} className="text-(--muted) group-hover:text-emerald-600 transition-colors" />
                            </div>
                        </div>
                    </Link>
                ))}
      ...
      <section className="section-dark py-24 px-6 text-white" aria-labelledby="blog-newsletter-heading">
         ...
         <div className="max-w-4xl mx-auto text-center relative z-10 px-4 sm:px-6 lg:px-8">
            <div className="w-16 h-16 bg-(--card)/10 rounded-2xl flex items-center justify-center mx-auto mb-8">
                <Mail size={32} />
            </div>
            ...
            <form className="max-w-md mx-auto flex flex-col sm:flex-row gap-4">
                <input
                    ...
                    className="flex-1 bg-(--card)/10 border border-white/10 rounded-xl px-6 py-4 text-white placeholder:text-(--muted) focus:outline-none focus:border-white transition-colors duration-(--duration-150)"
                />
                <Button type="submit" variant="primary" size="lg" className="!bg-(--emerald) !text-white hover:!opacity-90">
                    Subscribe
                </Button>
            </form>
            <p className="text-(--muted) text-xs mt-6">No spam. Unsubscribe anytime.</p>
         </div>
      </section>
